...
public class RedBlackTree {
    ...
    public void insertNode(Book insertionBook) {
        RedBlackTreeNode node = root;
        ...
        while (node != null) {
            parent = node;
            if (insertionBook.bookId < node.book.bookId) {
                node = node.left;
            } else if (insertionBook.bookId > node.book.bookId) {
                node = node.right;
            } else {
                throw new IllegalArgumentException("BST already contains a node with key " + insertionBook.bookId);
            }
        }

        // Insert new node
        RedBlackTreeNode newNode = new RedBlackTreeNode(insertionBook);
        newNode.color = RED;
        if (parent == null) {
            root = newNode;
        } else if (insertionBook.bookId < parent.book.bookId) {
            parent.left = newNode;
        } else {
